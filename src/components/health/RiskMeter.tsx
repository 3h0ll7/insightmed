...
interface RiskMeterProps { value?: number; label?: string; }
...
const RiskMeter = ({ value = 32, label = "Overall Risk" }: RiskMeterProps) => {
  ...
  return (
    <motion.div className="warm-card p-5 flex flex-col items-center"
      ...
      <h3 className="text-sm font-serif font-semibold text-foreground mb-4">{label}</h3>
      <div className="relative w-32 h-32">
        <svg viewBox="0 0 120 120" className="w-full h-full">
          ...
          <motion.circle cx="60" cy="60" r="54" fill="none" stroke={getColor(value)} strokeWidth="8" strokeLinecap="round"
            ...
            animate={{ strokeDashoffset: offset }} transition={{ delay: 1.6, duration: 1.2, ease: "easeOut" }}
            transform={`rotate(${rotation} 60 60)`} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <motion.span className="text-2xl font-bold text-foreground font-sans" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }}>
            {value}%
          </motion.span>
