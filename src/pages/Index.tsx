  ...
  const metabolicRisk = analysisData
    ? Math.round(
        analysisData.risk_factors
          .filter((r) => /glucose|diabetes|cholesterol|lipid|metabol|glycemic|triglyceride|hba1c/i.test(r.factor))
          .reduce((sum, r) => {
            const val = r.level === "high" ? 90 : r.level === "moderate" ? 55 : 20;
            return sum + val;
          }, 0) /
          Math.max(
            analysisData.risk_factors.filter((r) => /glucose|diabetes|cholesterol|lipid|metabol|glycemic|triglyceride|hba1c/i.test(r.factor)).length,
            1
          )
      )
    : 58;
    ...
    <div className="min-h-screen editorial-gradient">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        ...
        <div className="mb-10 max-w-3xl mx-auto">
          <SmartInputZone onProcessingChange={handleProcessingChange} onAnalysisComplete={handleAnalysisComplete} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 flex flex-col gap-6">
            <RiskMeter value={overallRisk} label={t("overallRisk")} animated={!!analysisData} />
            <RiskMeter value={metabolicRisk} label={t("metabolicRisk")} animated={!!analysisData} />
          </div>
          <div className="md:col-span-1">
